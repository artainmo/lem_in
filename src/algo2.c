...
static int collision(t_room ***prior_ants_paths, t_room *next, int it, t_antFarm *af)
{
  if (next == af->start_room || next == af->end_room)
    return 0;
  for (int i = 0; prior_ants_paths[i]; i++) {
    if (prior_ants_paths[i][it] == next)
      return 1;
  }
  return 0;
}
  ...
  for (int i = 0; prior_ants_paths[i] ; i++)
  {
    ...
    if (prior_ants_paths[i][_time+1] == next_room) //Create alternative path if ant collision occurs
    {
      ...
      for (int t = _time+1; prior_ants_paths[i][t]; t++) {
        if (collision(prior_ants_paths, prior_ants_paths[i][t], it, af))
          t--;
        add_room_end_array(alternative_paths[len], prior_ants_paths[i][t]);
        // view_path(alternative_paths[len], it);
        it++;
      }
